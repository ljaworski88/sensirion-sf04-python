...
with SMBus(3) as bus:
    ...
    try:
        product_name, product_serial, name_crc_result, serial_crc_result = read_product_info(bus, crc_check=False)
    ...
    try:
        user_reg_val, user_reg_crc, user_crc_result = read_user_reg(bus, crc_check=False)
    ...
    try:
        adv_reg_val, adv_reg_crc, adv_crc_result = read_adv_reg(bus, crc_check=False)
    ...
    for x in range(9,17):
        try:
            function_result = set_resolution(bus, x, crc_check=False)
    ...
    # for x in range(0, 5):
        # try:
            # function_result = set_calibration_field(bus, x, crc_check=False)
        ...
        # try:
            # scale_factor, units, scale_crc, unit_crc = read_scale_and_unit(bus, crc_check=False)
    ...
    for reading_type in ['flow', 'temp', 'vdd']:
        ...
        try:
            raw_data, data_crc, data_crc_result = read_raw_data(bus, crc_check=True)
        except Exception as e:
